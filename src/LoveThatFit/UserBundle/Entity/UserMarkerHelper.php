...
class UserMarkerHelper
{
    ...
    public function setArray($specs, $user_marker)
    {
        ...
        if (array_key_exists('coordinate_json', $specs) && $specs['coordinate_json']) {$user_marker->setCoordinateJson($specs['coordinate_json']);}
    ...
    public function getDefaultObject($user)
    {
        ...
        $user_marker->setImageActions(json_encode(array("move_up_down" => 0, "move_left_right" => 0, "img_rotate" => 0)));
    ...
    public function arrayToObject($user, $ar)
    {
        ...
        array_key_exists('image_actions', $ar) ? $user_marker->setImageActions($ar['image_actions']) : '';
    ...
    public function getComparisionArray($user)
    {
        ...
        foreach ($mm_specs['masked_marker'] as $mms_k => $mms_v) {
            ...
            ##added by umer bcz the fomula return the same values no calc required
    ...
    #-----------------------new method for support screen
    public function getDefaultMaskSupport($gender, $body_type)
    {
        $mask_type_array = $this->getMaskedMarkerSpecsSupport();
        if (array_key_exists($gender, $mask_type_array['mask_type'])) {
            if (array_key_exists($body_type, $mask_type_array['mask_type'][$gender])) {
                return $mask_type_array['mask_type'][$gender][$body_type];
            } else {
