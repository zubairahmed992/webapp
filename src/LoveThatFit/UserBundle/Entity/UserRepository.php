...
#--------------------- Get  User with Device Type ------------------------------#
    ...
    public function searchSupport($data, $page = 0, $max = null, $order, $getResult = true)
    {
        $query     = $this->getEntityManager()->createQueryBuilder();
        $search    = isset($data['query']) && $data['query'] ? $data['query'] : null;
        $gender    = isset($data['gender']) && $data['gender'] ? $data['gender'] : null;
    ...
    public function findUserListExportFInvited($start_date, $end_date)
    {
        ...
        $query->select('u.id, u.firstName, u.lastName, u.email, u.phoneNumber, 
                        u.gender,u.zipcode, u.createdAt')
            ->from('LoveThatFitUserBundle:User', 'u')
            ->innerJoin('LoveThatFitUserBundle:InviteFriend', 'i', 'WITH', 'i.user = u.id');
            if ($start != "" && $end != "") {
               $query->where('u.createdAt >= :start')
                    ->andWhere('u.createdAt < :end')
                    ->setParameter('start', $start)
    ...
    public function findUserListExporRMember($start_date, $end_date)
    {
        ...
        $query->select('u.id, u.firstName, u.lastName, u.email, u.phoneNumber, 
                        ...
                        u.friend_email')
            ->from('LoveThatFitUserBundle:User', 'u')
            ->where('u.friend_name IS NOT NULL');
            if ($start != "" && $end != "") {
               $query->andWhere('u.createdAt >= :start')
                    ->andWhere('u.createdAt < :end')
                    ->setParameter('start', $start)
                    ->setParameter('end', $end);
            }
