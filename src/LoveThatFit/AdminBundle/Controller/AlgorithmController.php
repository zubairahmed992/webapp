...
################################################################
    ...
    public function fitAlgorithm3CompareAction($user_id, $product_id, $json = 0) {
        $product = $this->get('admin.helper.product')->find($product_id);
        $user = $this->get('user.helper.user')->find($user_id);
        $fe = new FitAlgorithm3($user, $product);
        if ($json == 0) {
            return $this->render('LoveThatFitAdminBundle:Algoritm:_algo3_comparison.html.twig', array(
                        'product' => $product, 'user' => $user, 'data' => $fe->getFeedback(),
                    ));
        } elseif ($json == 1) {
            return new Response(json_encode($fe->getFeedback()));
...
//------------------------------------------------------------------------------------------
    ...
    public function userTestDemoProductsAction() {
        ...
        return $this->render('LoveThatFitAdminBundle:Algoritm:_recommendations_test_demo_products.html.twig', $arr);
    }
    #--------------------------------------------------
    ...
    #-------------------------------------------------- User Test Demo Products Ajax Call copy of Marathon
    public function userTestDemoProductListAction($user_id) {
        $arr=$this->test_demo_data($user_id);
        return $this->render('LoveThatFitAdminBundle:Algoritm:_recommendations_test_demo_product_list.html.twig', $arr);
    }
    #--------------------------------------------------
