...
//-----------------------------------------------------------------
    ...
    public function findByGenderBrand($gender, $brand_id, $page_number = 0, $limit = 0) {
        if ($page_number <= 0 || $limit <= 0) {
            ...
            JOIN p.brand b
            WHERE b.id = :id
            AND p.gender = :gender AND p.disabled=0'
...
//-----------------------------------------------------------------
    ...
    public function productListByBrand($brand_id, $gender) {
      ...
      WHERE
      p.gender = :gender
      AND b.id = :brand_id AND p.disabled=0"
    ...
    public function productListByClothingType($clothing_type_id, $gender) {
      ...
      WHERE
      p.gender = :gender
      AND ct.id = :clothing_type_id AND p.disabled=0"
 ...
 public function findProductByBrandWebService($id, $gender) {
        ...
        return $this->getEntityManager()
                        ->createQueryBuilder()
                        ->select('p.id as product_id,p.name,p.description,ct.target as target ,pc.image as product_image')
                        ->from('LoveThatFitAdminBundle:Product', 'p')
