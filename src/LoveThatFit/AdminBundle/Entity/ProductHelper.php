    ...
    public function searchProductByCriteria( $data ){
        ...
        foreach ($finalData as $fData) {
            $output['data'][] = [
                ...
                'status'    => ($fData['disabled'] == 0) ? "Enable" : "Disable",
...
#-------------------lattest Product Specificatiom-------------------#
    public function productDetailArray($data, $entity)
    {
        ...
        if (isset($data['product']['hem_length'])) {
            $entity->setHemLength($data['product']['hem_length']);
        }
        if (isset($data['product']['neckline'])) {
            $entity->setNeckLine($data['product']['neckline']);
        }
        if (isset($data['product']['sleeve_styling'])) {
            $entity->setSleeveStyling($data['product']['sleeve_styling']);
        }
        ...
        if (isset($data['fabric_content'])) {
            $entity->setFabricContent($this->getJsonForFields($data['fabric_content']));
        }
        if (isset($data['garment_detail'])) {
            $entity->setGarmentDetail($this->getJsonForFields($data['garment_detail']));
        }
...
#------------------------------#
    ...
    public function fillArray($size_type, $array, $gender = Null, $default_array)
    { #public function fillArray($size_type, $array, $default_array){
        $data = array();
        $fitType = $this->container->get('admin.helper.size')->getAllFitType();
        if (empty($array)) {
            $array = $default_array;
