...
#----------------------    
    ...
    public function generate($specs, $stretch_enabled=false) {
        ...
        foreach ($specs['sizes'] as $size => $fit_points) {
            foreach ($fit_points as $fpk => $fpv) {
                ...
                if(array_key_exists($fpk, $specs['fit_point_stretch']) && $specs['fit_point_stretch'][$fpk]>0){
                    ...
                    if(array_key_exists($fpk, $fpa['x']) && $fpa['x'][$fpk]>0){
                        $us['stretch_percentage'] = $fpa['x'][$fpk];
                    
                    }elseif(array_key_exists($fpk, $fpa['y']) && $fpa['y'][$fpk]>0){
                        $us['stretch_percentage'] = $fpa['y'][$fpk];
                    }
                }
        ...
        foreach ($specs_updated['sizes'][$fit_model_obj->getSize()] as $fit_point => $measure) {
                    ...
                    $grade_rule = $specs_updated['sizes'][$size][$fpk]['grade_rule'] + ($specs_updated['sizes'][$size][$fpk]['grade_rule'] * ($specs_updated['sizes'][$size][$fpk]['stretch_percentage']/100));    
                    

                    $specs_updated['sizes'][$fit_model_obj->getSize()][$fit_point]['min_calc'] = $fit_model_fit_points[$fit_point] > 0 ? number_format($fit_model_fit_points[$fit_point] - (2.5 * $grade_rule), 2, '.', '') : 0;
                    ...
                    $specs_updated['sizes'][$fit_model_obj->getSize()][$fit_point]['ideal_low'] = $fit_model_fit_points[$fit_point] > 0 ? number_format($fit_model_fit_points[$fit_point] - (0.5 * $grade_rule), 2, '.', '') : 0;
                    
                    $specs_updated['sizes'][$fit_model_obj->getSize()][$fit_point]['fit_model'] = $fit_model_fit_points[$fit_point];
                    $specs_updated['sizes'][$fit_model_obj->getSize()][$fit_point]['ideal_high'] = $fit_model_fit_points[$fit_point] > 0 ? number_format($fit_model_fit_points[$fit_point] + (0.5 * $grade_rule), 2, '.', '') : 0;
                    
                    $specs_updated['sizes'][$fit_model_obj->getSize()][$fit_point]['max_calc'] = $fit_model_fit_points[$fit_point] > 0 ? number_format($fit_model_fit_points[$fit_point] + (2.5 * $grade_rule), 2, '.', '') : 0;
            ...
            $fit_model_ratio[$fit_point]['min_calc'] = ($measure['garment_dimension'] > 0 ) ? ($specs_updated['sizes'][$fit_model_obj->getSize()][$fit_point]['min_calc'] / $measure['garment_dimension']) : 0;            


            $fit_model_ratio[$fit_point]['ideal_low'] = ($measure['garment_dimension'] > 0 ) ? ($specs_updated['sizes'][$fit_model_obj->getSize()][$fit_point]['ideal_low'] / $measure['garment_dimension']) : 0;            

            $fit_model_ratio[$fit_point]['fit_model'] = ($measure['garment_dimension'] > 0 ) ? ($fit_model_fit_points[$fit_point] / $measure['garment_dimension']) : 0;
            
            $fit_model_ratio[$fit_point]['ideal_high'] = ($measure['garment_dimension'] > 0 ) ? ($specs_updated['sizes'][$fit_model_obj->getSize()][$fit_point]['ideal_high'] / $measure['garment_dimension']) : 0;            

            $fit_model_ratio[$fit_point]['max_calc'] = ($measure['garment_dimension'] > 0 ) ? ($specs_updated['sizes'][$fit_model_obj->getSize()][$fit_point]['max_calc'] / $measure['garment_dimension']) : 0;            
        ...
        foreach ($specs_updated['sizes'] as $size => $fit_points) {
            ...
            $specs_updated['sizes'][$size][$fpk]['min_calc'] = $specs_updated['sizes'][$size][$fpk]['garment_dimension'] * $fit_model_ratio[$fit_point]['min_calc'] ;                
            
            $specs_updated['sizes'][$size][$fpk]['min_actual'] =$specs_updated['sizes'][$size][$fpk]['min_calc'] ;  
            
            $specs_updated['sizes'][$size][$fpk]['ideal_low'] = $specs_updated['sizes'][$size][$fpk]['garment_dimension'] * $fit_model_ratio[$fit_point]['ideal_low'] ;                
            
            $specs_updated['sizes'][$size][$fpk]['fit_model'] = $specs_updated['sizes'][$size][$fpk]['garment_dimension'] * $fit_model_ratio[$fit_point]['fit_model'] ;                
            
            $specs_updated['sizes'][$size][$fpk]['ideal_high'] = $specs_updated['sizes'][$size][$fpk]['garment_dimension'] * $fit_model_ratio[$fit_point]['ideal_high'] ;                

            $specs_updated['sizes'][$size][$fpk]['max_calc'] = $specs_updated['sizes'][$size][$fpk]['garment_dimension'] * $fit_model_ratio[$fit_point]['max_calc'] ;
