...
#~~~~~~~~~~~~~~~~~~~ ws_misc_banner   /ws/misc_banner
    ...
    public function shopTheLookV3Action(Request $request)
    {
        $decoded = $request->request->all();
        $decoded = $this->get('webservice.helper')->processRequest($this->getRequest());
        $shopLook = $this->get('admin.helper.shoplook')->findAll();
    ...
    #----------------------- get order sales tax ------------------------------------------
    public function getOrderSalesTaxUserAction_bkp(Request $request) {
        $decoded = $request->request->all();
        ...
        if ($user) {
            if(isset($decoded['shipment_address']) && !empty($decoded['shipment_address'])) {
                ...
                $res = $this->get('webservice.helper')->response_array(true, 'Order sales tax', true, array(
