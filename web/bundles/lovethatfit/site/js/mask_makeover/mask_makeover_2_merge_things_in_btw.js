...
$(document).ready(function() {
   ...
   lyr_area_hldr.name = "lyr_area_hldr";
...
var active_hair_items = {
    hair_selection_active: false,
    hair_shape_init: true,
    segment: -1,
    drag: false,
    handleIn_drag: false,
    handleOut_drag: false
}
var recent_items = {
    mask: []
}
...
function init(){
    ...
    user_image.on('load', function() {
        ...
        user_img_hair = user_image.clone();
        user_img_hair.pivot = new Point(-user_img_hair.width/2, -user_img_hair.height/2);
        user_img_hair.position = new Point(50,50);
        user_img_hair.visible = false;
        
        hair_shape = new Path();
        hair_shape.strokeColor = 'green';
        hair_shape.fullySelected = true;
        hair_shape.visible = false;
        
        clip_shape = hair_shape;
        
        
        
        grp_img_hair_items = new Group(clip_shape, user_img_hair,hair_shape);
        //grp_img_hair_items.clipped = true;
        alert(grp_img_hair_items.layer.name);
...
function preset_user_image(move_up_down, move_left_right, img_rotate) {
    ...
    grp_img_hair_items.position.y = grp_img_hair_items.position.y += move_up_down;
    grp_img_hair_items.position.x = grp_img_hair_items.position.x += move_left_right;
    grp_img_hair_items.rotate(img_rotate); // -0.1 for left, 0.1 for right
}
...
function move_left(){
    ...
    if(current_status.zoom){
        ...
        grp_img_hair_items.position.x -= 0.5;
        image_actions_count.move_left_right -= 0.5;
    }else{
        alert(grp_img_hair_items.position.x);
        user_image.position.x -= 1;
        grp_img_hair_items.position.x -= 1;
...
function onMouseDown(event) {
    ...
    if(hitResult.type == "pixel"){
        //alert(hitResult.color);
        console.log(color_meter_rgb(hitResult.color.red,hitResult.color.green,hitResult.color.blue,0.2,0.16471,0.16863));
        //alert(color_meter("#0053c4","#006ace"));
    }
    if(hitResult.type == "segment" && !active_hair_items.hair_selection_active){
    ...
    /////////////////////////// Hair Selection //////////////////////////////////
  if(active_hair_items.hair_selection_active){
        if(hair_shape.closed){
            console.log(hitOptions);
            if(hitResult.type == "segment"){
                for(i=0; i<hair_shape.segments.length; i++){
                    if(hair_shape.segments[i].point == hitResult.segment.point){
                        active_hair_items.segment = hair_shape.segments[i];
                    }
                }
                active_hair_items.drag = true;
                active_hair_items.segment.selected = true;
                
                
            }
            if(hitResult.type == "handle-in"){
                active_hair_items.drag = false;
                active_hair_items.handleIn_drag = true;
                active_hair_items.segment = hitResult.segment;
            }
            if(hitResult.type == "handle-out"){
                active_hair_items.drag = false;
                active_hair_items.handleOut_drag = true;
                active_hair_items.segment = hitResult.segment;
            }

                    
//            if(hitResult.type == "stroke"){
//                var loc_add_point = hair_shape.getNearestLocation(event.point);
//                alert()
//                hair_shape.splitAt(loc_add_point);
//                console.log("aa gaya");
////                hair_shape.lastSegment.selected = true;	
//                
//            }
            ...
            hair_seg = new Segment({point: event.point, handleIn: [0,0], handleOut: [0,0]});
            hair_shape.add(hair_seg);
            hair_selection_act_seg = hair_shape.segments[hair_shape.segments.length - 1];

            hair_shape.fullySelected = true;
            if(Key.isDown('control')){
                hair_shape.closed = true;
                hair_shape.fullySelected = false;
                hair_shape.selected= true;
//               group_hair_items = new Group(hair_shape, user_img_hair);
//                group_hair_items.clipped = true;
            }
        }
     }
     //////////////////////// End hair selection //////////////
...
function onMouseDrag(event) {
    ...
    console.log("Hair Status: " + active_hair_items.hair_selection_active);
    ...
    if(active_items.drag){
        ...
        if(active_hair_items.hair_selection_active){
            active_hair_items.segment.point = event.point;
            circle_in.position = new Point(active_hair_items.segment.point.x + active_hair_items.segment.handleIn.x, active_hair_items.segment.point.y + active_hair_items.segment.handleIn.y);
            circle_out.position = new Point(active_hair_items.segment.point.x + active_hair_items.segment.handleOut.x, active_hair_items.segment.point.y + active_hair_items.segment.handleOut.y);
        }
    }
    ...
    if(active_hair_items.hair_selection_active){
        if(!hair_shape.closed){
            hair_selection_act_seg.handleIn.x -= event.delta.x;
            hair_selection_act_seg.handleIn.y -= event.delta.y;
            hair_selection_act_seg.handleOut.x += event.delta.x;
            hair_selection_act_seg.handleOut.y += event.delta.y;
        }
        if(active_hair_items.drag){
            active_hair_items.segment.point.x += event.delta.x;
            active_hair_items.segment.point.y += event.delta.y;
        }
        alert(hitResult.type);
        if(active_hair_items.handleIn_drag){
            active_hair_items.segment.handleIn.x += event.delta.x;
            active_hair_items.segment.handleIn.y += event.delta.y;
        }
        if(active_hair_items.handleOut_drag){
            active_hair_items.segment.handleOut.x += event.delta.x;
            active_hair_items.segment.handleOut.y += event.delta.y;
        }
...
function hair_selection(){
    //alert(project.activeLayer);
    active_hair_items.hair_selection_active = true;
//    lyr_area_hldr.visible = false;
//    lyr_hair_selection_path.visible = true;
//    lyr_hair_selection_path.activate();
    
    if(active_hair_items.hair_shape_init){
        hair_init();
    }else{
        edit_hair_shape();
    }
...
function hair_init(){
    user_img_hair.visible = true;
    hair_shape.visible = true;
    
    full_mask.visible = false;
}
function clip_hair_selection() {
    grp_img_hair_items.clipped = true;
    alert(grp_img_hair_items.layer);
}
function back_to_calibration(){
    active_hair_items.hair_selection_active = false;
    user_img_hair.visible = false;
    hair_shape.visible = false;
    view.update();
}
...
//following functions are not required for this time
function get_hair_area(){
    hair_area = new Rectangle(400, 150, 250, 250);
    hair_area_data = getImageData(hair_area);
    alert(hair_area_data);
}
...
function color_meter_rgb(r1,g1,b1,r2,g2,b2) {
    var _r = r1 * 100;
    var _g = g1 * 100;
    var _b = b1 * 100;
