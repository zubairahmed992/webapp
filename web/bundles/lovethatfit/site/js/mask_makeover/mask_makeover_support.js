...
/****** - Get max hip - ******/
    edge_grabber_1 =  new Path({
     ...
     edge_grabber_1.visible = false;
     body_part_area = new Path();
     console.log(body_part_max_width());
/****** - /Get max hip - ******/
...
function onMouseUp(event) {
    ...
    active_items.cir_out = false;
    h_indi.visible = false;
//    if(active_items.segment.path.data.name=="head_line"){
//
//        
...
function body_part_max_width(){
  if(body_part_area){body_part_area.removeSegments();}
  body_part_area.add(full_mask.segments[21]);
  body_part_area.add(full_mask.segments[22]);
  body_part_area.add(full_mask.segments[48]);
  body_part_area.add(full_mask.segments[49]);
  ...
  function get_lower_seg(seg_1_y, seg_2_y){
    ...
    if(seg_1_y <= seg_2_y){
        return lower_seg = seg_2_y;
    }else{
        return lower_seg = seg_1_y;
    }
  }

  function get_higher_seg(seg_1_y, seg_2_y){
    if(seg_1_y <= seg_2_y){
        return higher_seg = seg_2_y;
    }else{
        return higher_seg = seg_1_y;
    }
  }
  //alert(get_lower_seg(full_mask.segments[49].point.y,full_mask.segments[21].point.y));
  var body_part_y = [];
  var arr_incr = 0;
  //alert(body_part_area.segments[0].point.y);
  var start_y = get_lower_seg(body_part_area.segments[3].point.y,body_part_area.segments[0].point.y);
  var end_y = get_higher_seg(body_part_area.segments[2].point.y,body_part_area.segments[1].point.y);
//alert(start_y +" --- "+end_y);
var curr_wide = 1;
var last_wide = 0;
var max_widest;
  ...
  //for (var i = 600; i < 800; i++) {
    //alert(body_part_area +" + "+ edge_grabber);

    edge_grabber_1.position.y = i;
    body_part_y[arr_incr] = get_max_hip(body_part_area, edge_grabber_1);
    curr_wide = body_part_y[arr_incr].mbp_widest;
    if(curr_wide > last_wide){
      max_widest = body_part_y[arr_incr];
      last_wide = curr_wide;
    }
  ...
  //alert(body_part_y);
  function get_max_hip(mbp, mbp_y_line){
    var mbp_inter_data = mbp.getIntersections(mbp_y_line);
    rawana_info = {
      mbp_y: mbp_inter_data[0].point.y,
      mbp_x1: mbp_inter_data[0].point.x,
      mbp_x2: mbp_inter_data[1].point.x,
      mbp_widest: mbp_inter_data[0].point.x - mbp_inter_data[1].point.x
    }
    //alert(user_mask1.segments +"  >>>");
      return rawana_info;
...
////////////////////////********* Additional Points Extract ******** //////////////////


function get_additional_points_y_pos(user_mask, edge_grabber){
  var grabed_edges = user_mask.getIntersections(edge_grabber);
    return grabed_edges;
    //alert(user_mask.segments);
}

function get_additional_pos(){

    var all_additional_points_y_pos = [];
    var incr = 0;
    var flaging = true;

    for (var i = 0; i < 800; i++) {
        //console.log(path_line.position.y);
    	// Each frame, rotate the path by 3 degrees:
    	if(edge_grabber.position.y <= 800){
    	edge_grabber.position.y += 10;
    	incr++;
